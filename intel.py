...
def df_to_dataset(dataframe, shuffle=True, batch_size=32, input_cols=['blob_x', 'blob_y'],
  ...
  return ds, input_array, output_array
...
def model_update(expl_rate=0.2, move_limit=20, coord_range=4, batch_size=32, n_epochs=5, test_split=0.2, val_split=0.2, order_strategy='random', response_cols=['sim_result'], input_cols=['blob_x', 'blob_y', 'target_x', 'target_y'], classification=True, version_lookback=math.inf):
    ...
    val_ds, val_input, val_output  = df_to_dataset(val, shuffle=False, batch_size=batch_size, input_cols=input_cols,
                           response_cols=response_cols, coord_range=coord_range, classification=classification)
    test_ds, test_input, test_output = df_to_dataset(test, shuffle=False, batch_size=batch_size, input_cols=input_cols,
                            response_cols=response_cols, coord_range=coord_range, classification=classification)
    ...
    if intel_version == 1:
        ...
        model = tf.keras.Sequential([
                ...
                #feature_layer,
                tf.keras.layers.Flatten(input_shape=train_input.shape),
    ...
    if mcp_save.best <= persist_best_score:
        ...
        # store intel version
        with open('./intel/%s.intel_version.pickle' %(file_pattern), 'wb') as f:
            pickle.dump(intel_version, f, protocol=pickle.HIGHEST_PROTOCOL)
        # save model object
